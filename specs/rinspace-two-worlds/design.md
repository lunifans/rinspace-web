...
> 状态：技术设计、任务 1–27 与生产切换已完成；联邦保持关闭。
...
| 组件                     | 权威职责                                                                   | 不承担的职责                                    |
| ------------------------ | -------------------------------------------------------------------------- | ----------------------------------------------- |
| `rinspacehq/rinspace-web`  | 表世界 UI、共享顶栏包、世界路由契约、公共 OpenAPI、确定性 Demo、设计 token | 生产密钥、Mastodon 数据、私有部署               |
| `rinspacehq/mastodon`      | 里世界 UI、帖子/互动/通知/列表、`/p` 地址、浏览量、推荐适配、严格本地模式  | 表世界正文、Gitea 内容权威、Rinspace 登录主数据 |
| 私有 `rinspacehq/rinspace` | 同源网关、OIDC/账号预配、资料和标签绑定、关系迁移、制品锁、生产配置        | 长期复制公开前端源码、并行维护第二套社交图      |
...
- 个性化推荐具备显著告知、关闭、非个性化替代、标签管理和投诉能力；适用备案/评估由负责人持续跟踪，不作为自动化发布门禁；
...
2026-09-05 的实际生产执行由唯一负责人批准采用快速切换：在受保护的 loopback 预演、不可变制品验收和回退验证通过后，压缩按自然日计算的观察窗口并直接进入公开阶段；写入、推荐和浏览量开启，联邦保持关闭。算法备案证明材料作为上线后跟踪事项，不参与本次切换判定。
