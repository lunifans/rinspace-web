...
test('a previous complete core can be restored after a newer package is assembled', () => {
  ...
  writeCore(coreV2, 'v2');
  fs.mkdirSync(output);
  const outputInode = fs.statSync(output).ino;
  ...
  if (process.platform !== 'win32') assert.equal(fs.statSync(output).ino, outputInode, 'assembly must preserve an existing output mount point');
